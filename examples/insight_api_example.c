...
int main(int argc, char *argv[]) {
    ...
    if (get_read_latency_percentiles(device, time_start, time_end, 0ULL, 0ULL, INSIGHT_JSON_QUERY_SESSION_ID_NONE, json_buffer) != 0) {
        fprintf(stderr, "get_read_latency_percentiles failed: %s\n", strerror(errno));
        return 2;
    }
    printf("=== read_latency_percentiles ===\n%s\n\n", json_buffer);

    if (get_write_latency_percentiles(device, time_start, time_end, 0ULL, 0ULL, INSIGHT_JSON_QUERY_SESSION_ID_NONE, json_buffer) != 0) {
        fprintf(stderr, "get_write_latency_percentiles failed: %s\n", strerror(errno));
        return 14;
    }
    printf("=== write_latency_percentiles ===\n%s\n\n", json_buffer);

    if (get_write_amplification(device, time_start, time_end, 0ULL, 0ULL, INSIGHT_JSON_QUERY_SESSION_ID_NONE, json_buffer) != 0) {
        fprintf(stderr, "get_write_amplification failed: %s\n", strerror(errno));
        return 3;
    }
    printf("=== write_amplification ===\n%s\n\n", json_buffer);

    if (get_qd_distribution(device, time_start, time_end, 0ULL, 0ULL, INSIGHT_JSON_QUERY_SESSION_ID_NONE, json_buffer) != 0) {
        fprintf(stderr, "get_qd_distribution failed: %s\n", strerror(errno));
        return 4;
    }
    printf("=== qd_distribution ===\n%s\n\n", json_buffer);

    if (get_read_size_distribution(device, time_start, time_end, 0ULL, 0ULL, INSIGHT_JSON_QUERY_SESSION_ID_NONE, json_buffer) != 0) {
        fprintf(stderr, "get_read_size_distribution failed: %s\n", strerror(errno));
        return 5;
    }
    printf("=== read_size_distribution ===\n%s\n\n", json_buffer);

    if (get_write_size_distribution(device, time_start, time_end, 0ULL, 0ULL, INSIGHT_JSON_QUERY_SESSION_ID_NONE, json_buffer) != 0) {
        fprintf(stderr, "get_write_size_distribution failed: %s\n", strerror(errno));
        return 6;
    }
    printf("=== write_size_distribution ===\n%s\n\n", json_buffer);

    if (get_read_throughput_distribution(device, time_start, time_end, 0ULL, 0ULL, INSIGHT_JSON_QUERY_SESSION_ID_NONE, json_buffer) != 0) {
        fprintf(stderr, "get_read_throughput_distribution failed: %s\n", strerror(errno));
        return 7;
    }
    printf("=== read_throughput_distribution ===\n%s\n\n", json_buffer);

    if (get_write_throughput_distribution(device, time_start, time_end, 0ULL, 0ULL, INSIGHT_JSON_QUERY_SESSION_ID_NONE, json_buffer) != 0) {
    ...
    if (get_read_count_distribution(device, block_size, time_start, time_end, 0ULL, 0ULL,
                                      INSIGHT_JSON_QUERY_SESSION_ID_NONE, json_buffer) != 0) {
    ...
    if (get_lifecycle_distribution(device, block_size, time_start, time_end, 0ULL, 0ULL, INSIGHT_JSON_QUERY_SESSION_ID_NONE, json_buffer) !=
        0) {
    ...
    if (get_nand_write_volume(device, time_start, time_end, 0ULL, 0ULL, INSIGHT_JSON_QUERY_SESSION_ID_NONE, json_buffer) != 0) {
        fprintf(stderr, "get_nand_write_volume failed: %s\n", strerror(errno));
        return 12;
    }
    printf("=== nand_write_volume ===\n%s\n\n", json_buffer);

    if (get_gc_data_movement(device, time_start, time_end, 0ULL, 0ULL, INSIGHT_JSON_QUERY_SESSION_ID_NONE, json_buffer) != 0) {
