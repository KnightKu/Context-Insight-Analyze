...
```bash
./sfx_ctx_insight_analyze [-D|--debug] [-j|--format-json] [-l|--latency] [-r|--read-count] [-w|--w2fr] [-c|--life-cycle] [-q|--qd-dist] [-a|--wa-dist] [-R|--read-size-dist] [-W|--write-size-dist] [-T|--trim-size-dist] [-S|--time-start "YYYY-MM-DD HH:MM:SS"] [-E|--time-end "YYYY-MM-DD HH:MM:SS"] [-b|--block-size N[K|M|G|T]] <device_name> <slba[K|M|G|T]> <data_len[K|M|G|T]>
```
...
- `-b` / `--block-size`: filters `read/write/trim` records by I/O size. Records are dropped when
  size is smaller than `block-size` or not an integer multiple of `block-size`.
...
./sfx_ctx_insight_analyze --latency --read-size-dist --block-size 64K /dev/nvme0n1 0 64M
...
- `get_write_amplification(...)` uses `block_size=0` internally (no block-size filtering)
- `get_qd_distribution(...)` ~=  
  `./sfx_ctx_insight_analyze --format-json --qd-dist -S <time_start> -E <time_end> <device> 0 11T`
- `get_read_size_distribution(...)` ~=  
  `./sfx_ctx_insight_analyze --format-json --read-size-dist -S <time_start> -E <time_end> <device> 0 11T`
- `get_write_size_distribution(...)` ~=  
  `./sfx_ctx_insight_analyze --format-json --write-size-dist -S <time_start> -E <time_end> <device> 0 11T`
- `get_read_throughput_distribution(...)` ~=  
  `./sfx_ctx_insight_analyze --format-json --read-throughput-dist -S <time_start> -E <time_end> <device> 0 11T`
