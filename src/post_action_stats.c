...
static int build_lba_ratio_summary_locked(nvme_post_action_lba_ratio_summary_t *out) {
    ...
    memcpy(out->read_count_bytes_hist, g_read_req_bytes_hist, sizeof(out->read_count_bytes_hist));
    memcpy(out->life_cycle_bytes_hist, g_life_cycle_req_bytes_hist,
           sizeof(out->life_cycle_bytes_hist));
    return 0;
}
...
static void print_ratio_hist_line_with_bytes(const char *name,
                                             uint64_t hit,
                                             uint64_t total,
                                             const char *label,
                                             uint64_t bytes) {
...
static void print_ratio_hist_line_json(const char *label,
    ...
    fprintf(stderr,
            "      {\"label\":\"%s\",\"count\":%" PRIu64 ",\"ratio\":%.2f}%s\n",
            label,
            hit,
            pct,
            with_comma != 0 ? "," : "");
...
static void print_ratio_hist_line_with_bytes_json(const char *label,
                                                  uint64_t hit,
                                                  uint64_t total,
                                                  uint64_t bytes,
                                                  int with_comma) {
...
void nvme_post_action_stats_update_write(uint64_t start_lba,
    ...
    if (track_read_count == 0 && track_w2fr == 0 && track_life_cycle == 0) {
        pthread_mutex_unlock(&g_stats_mutex);
        return;
    }
    uint64_t begin = 0ULL;
    ...
    if (track_w2fr == 0 && track_life_cycle == 0) {
    ...
    for (uint64_t idx = begin; idx < end_exclusive; ++idx) {
        ...
        if (track_life_cycle != 0 &&
            !is_new && w->last_write_abs_us > 0ULL && abs_time_us >= w->last_write_abs_us) {
    ...
    uint64_t req_bytes = (end_exclusive - begin) * g_stats_block_bytes;
        ...
        g_life_cycle_req_bytes_hist[life_idx] += req_bytes;
...
void nvme_post_action_stats_update_read(uint64_t start_lba,
    ...
    if (track_read_count == 0 && track_w2fr == 0) {
        pthread_mutex_unlock(&g_stats_mutex);
        return;
    }
    uint64_t begin = 0ULL;
    ...
    for (uint64_t idx = begin; idx < end_exclusive; ++idx) {
        ...
        if (track_read_count != 0 && s->read_count < 255U) {
            ++s->read_count;
        }
        if (track_w2fr != 0 &&
            w->first_read_seen == 0U && abs_time_us >= w->last_write_abs_us) {
            uint32_t d_ms = duration_us_to_ms(abs_time_us - w->last_write_abs_us);
...
int nvme_post_action_stats_record_stat(uint64_t abs_time_us,
    ...
    if (qd == 0U && hot_write_4k == 0U && folding_write_4k == 0U) {
        pthread_mutex_unlock(&g_stats_mutex);
        return 0;
    }
